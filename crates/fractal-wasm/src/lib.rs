...
#[allow(
    ...
    reason = "wasm-bindgen exports cannot be marked `unsafe` while remaining callable from JS; the JS-side caller upholds the (ptr, len) pairing invariant described in this function's doc comment. The render-layer cache in `web/src/worker/handler.ts` encodes that invariant explicitly."
)]
pub fn colorize(
    iter_ptr: *const f32,
    len: usize,
...
#[allow(
    clippy::not_unsafe_ptr_arg_deref,
    reason = "wasm-bindgen exports cannot be marked `unsafe` while remaining callable from JS; the caller upholds the (iter_ptr, len) pairing invariant exactly as `colorize` documents — the render-layer cache in `web/src/worker/handler.ts` enforces it."
)]
pub fn probe_pixel(
    iter_ptr: *const f32,
    len: usize,
